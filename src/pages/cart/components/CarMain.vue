...
const onChangeCount = async (ev: InputNumberBoxEvent) => {
  ...
  await putMemberCartByskuIdAPI(index, { count: value })
}
//修改购物车商品选中状态
...
//计算购物车选中商品总金额
const selectTotalPrice = computed(() => {
      ...
      <view class="toolbar" :style="{ paddingBottom: SafeArea + 'px' }">
        ...
        <view class="button-grounp">
          <view
            ...
            :class="{ disabled: selectTotalCount === 0 }"
          >
            去结算({{ selectTotalCount }})
          </view>
        </view>
      </view>
    </template>
